def resolve_paths(d):
    ...
    if typ == "auto":
        if source == "rs":
            ...
            yield_path = f"data/yields/yields_auto_rs_P_v{autonet_id}_{yield_mode}.pkl"
        else:
            net_path   = f"data/networks/autonets_mp_{pruner}_{suffix}_pv{pv}_v{autonet_id}.pkl"
            yield_path = f"data/yields/yields_auto_mp_{pruner}_{suffix}_pv{pv}_v{autonet_id}_{yield_mode}.pkl"
    else:
        if source == "rs":
            net_path   = f"data/networks/crossnets_rs_P_v{autonet_id}_{met}_v{crossnet_id}.pkl"
            yield_path = f"data/yields/yields_cross_rs_P_v{autonet_id}_{met}_v{crossnet_id}_{yield_mode}.pkl"
        else:
            net_path   = f"data/networks/crossnets_mp_{pruner}_{suffix}_pv{pv}_v{autonet_id}_{met}_v{crossnet_id}.pkl"
            yield_path = f"data/yields/yields_cross_mp_{pruner}_{suffix}_pv{pv}_v{autonet_id}_{met}_v{crossnet_id}_{yield_mode}.pkl"
